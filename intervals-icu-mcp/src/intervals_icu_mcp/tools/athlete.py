...
async def get_fitness_summary(
    ...
    try:
        async with ICUClient(config) as client:
            ...
            # CTL/ATL/TSB live on the wellness endpoint, not the athlete endpoint.
            # Try today, fall back up to 7 days to find the most recent record with data.
            wellness = None
            for days_back in range(8):
                check_date = (date.today() - timedelta(days=days_back)).isoformat()
                try:
                    w = await client.get_wellness_for_date(check_date)
            ...
            if wellness is None or (wellness.ctl is None and wellness.atl is None):
                return ResponseBuilder.build_error_response(
